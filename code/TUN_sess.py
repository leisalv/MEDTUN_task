...
class TuningSession(PylinkEyetrackerSession):
    ...
    def __init__(self, output_str, output_dir=None, settings_file=None, sequence_id = None, n_trials=None, eyetracker_on=True, flicker = False, photodiode_check = False, debug = False):
        ...
        # size1 = tools.monitorunittools.deg2pix(self.default_fix.size, self.monitor)
        # size2 = self.default_fix2.size
        # size2 = tools.monitorunittools.pix2deg(self.default_fix2.getSize())

        # print(f"sizes in pixels: {size1}, {size2}")
        ...
        print(f'total actual TRs (1.6): {self.total_TRs_16}')
    ...
    def create_trials(self, timing='frames'):
        ...
        stim_durations_p0 = self.trial_sequence_df['phase_0_frames'] 
        ...
        iti_durations_p1 = self.trial_sequence_df['phase_1_frames'] 

        # make phase durations list of tuples for prep, iti, trial
        # phase_durations = list(zip(prep_durations_p0, stim_durations_p0, iti_durations_p1))
        phase_durations = list(zip(stim_durations_p0, iti_durations_p1))
        ...
        dummy_start = TuningTrial(session=self,
                        #   trial_nr=self.n_trials, # sets it to +1 the last trial number
                          trial_nr='dummy_start',
                          phase_durations=(0, int(self.settings['stimuli']['blank_before_trs']* self.TR * 120)),
                          txt='Trial %i:' % trial_nr,
        ...
        dummy_end = TuningTrial(session=self,
                        #   trial_nr=self.n_trials, # sets it to +1 the last trial number
                          trial_nr='dummy_end',
                          phase_durations=(0, int(self.settings['stimuli']['blank_after_trs']* self.TR * 120)),
                          txt='Trial %i' % trial_nr,
                          parameters=dict(trial_type = 'dur',
                                            stim_dur = 0, 
                                            texture_path = self.texture_paths[0]),
